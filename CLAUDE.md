...
4. **Coleções**
   ...
   - **Sub-coleções (hierarquia leve, um nível só — adiantado de v2, feedback real de uso)**: matérias grandes viravam coleções extensas demais pra estudar de uma vez. Decisão final, com escopo reduzido:
     - `collections.parent_id` (auto-referência, nullable) — uma coleção pode ter uma coleção "mãe". **Só um nível permitido**: uma coleção que já tem `parent_id` não pode virar mãe de outra (validar no servidor, não só na UI)
     - **Cards sempre vivem na coleção-filha** (ou numa coleção sem hierarquia nenhuma) — a coleção-mãe é um contêiner puramente visual de agrupamento, nunca tem card direto
     - **Sem agregação de estudo**: estudar continua sempre por coleção específica (a "aula"), exatamente como já funciona hoje — **não** existe "estudar a matéria inteira" agregando vencidos de todas as filhas (decisão consciente, mantém o SM-2 e a lógica de estudo intocados)
     - Lista de Coleções agrupa visualmente: coleção-mãe aparece como cabeçalho expansível, filhas listadas dentro
     - Definir/trocar a coleção-mãe de uma coleção existente é uma ação separada (não durante criação via `DestinationPicker`, que continua criando sempre coleções de nível único/simples) — mesmo espírito da ação "trocar coleção do card" já implementada no modo de navegação, só que aplicada à coleção em si
...
9. **Tela "Não organizados"** (v1, pipeline de IA já pronto — implementar agora)
   ...
   - **A IA nunca inventa coleção nova** — a sugestão é sempre dentre as coleções já existentes do usuário, ou `null` se nenhuma fizer sentido. Criar coleção nova continua sendo decisão da usuária, não da IA.
   ...
   - **Três ações por card, não duas**: (1) tocar na coleção sugerida pela IA para mover o card pra lá (ação rápida, um toque); (2) **escolher manualmente** outra coleção — reaproveitar o mesmo `DestinationPicker` já usado em CSV/geração (coleção existente diferente da sugerida, criar nova, ou deixar sem coleção), para quando a usuária discordar da sugestão ou quiser outra opção; (3) remover o card diretamente. A sugestão da IA é um atalho de conveniência, nunca a única forma de decidir.
   - **"Aplicar todas as sugestões"** (botão em lote, feedback real de uso): move de uma vez todos os cards que têm sugestão da IA (não-null) pra suas respectivas coleções sugeridas — cards sem sugestão continuam na lista pra tratamento manual. Pede confirmação antes (mostrando quantos cards serão movidos), já que afeta vários cards de uma vez, mesmo não sendo ação destrutiva (sempre dá pra mover de novo depois). Trata falha parcial: se mover um card específico falhar, não deve invalidar silenciosamente os que já tiveram sucesso — reportar quantos moveram e quais falharam, se houver.
   - Se a usuária não tiver nenhuma coleção ainda (todas as coleções foram apagadas, ou nunca criou nenhuma), não há o que sugerir — pular a chamada de IA e tratar como "sem sugestão disponível" (mostrar só o DestinationPicker manual — criar coleção nova — ou remover)
...
- **Compartilhar coleções com outros usuários** (decisão de direção tomada: opção cópia, via link público): dono clica "Compartilhar" numa coleção, gera um link público único. Quem recebe o link pode **visualizar** os cards em modo leitura sem precisar de login (preview da coleção compartilhada), mas **importar exige login** (só o dono do destino grava dado, então precisa saber quem é — se não estiver logado, prompt pra login/cadastro antes de completar a importação). Ao importar, os cards são **copiados** (duplicados) pra conta do destinatário — cópia independente, sem sincronia com o original depois (dono edita a coleção original, cópia do destinatário não muda).
  - Arquitetura: tabela nova `collection_shares` (não campo direto em `collections`) — permite múltiplos links por coleção ao longo do tempo, e o dono poder **revogar/regenerar** o link sem afetar a coleção em si (campo `revogado`/token novo invalida o anterior). Token único e não-adivinhável (não sequencial) como identificador da URL pública.
  - Importação reaproveita o `DestinationPicker` já existente (escolher coleção existente do destinatário, criar nova, ou sem coleção) — mesmo padrão de todo o resto do app.
  - Cards importados por esse caminho podem ganhar um novo valor de `origem` (`"compartilhado"`, além dos já existentes `ia`/`csv`/`manual`) para rastreabilidade, se fizer sentido na hora.
  - Rota pública de preview não deve expor nenhum dado do dono além do necessário (nome da coleção, cards) — não vazar email, estatísticas pessoais, streak, etc. do dono original.
- **Melhorar/otimizar card via IA no modo de edição (navegação/browse)**: botão "Melhorar com IA" na tela de edição de card (modo navegação) — envia frente/verso atuais para a API (reaproveitando a mesma infraestrutura e diretrizes de qualidade já usadas na geração: atomicidade, clareza, evitar trivialidade), retorna uma versão sugerida que a usuária revisa e aceita ou descarta antes de salvar — nunca sobrescreve automaticamente. Mesmo isolamento do SM-2 já garantido no modo de edição continua valendo (só toca em `flashcards.frente`/`verso`, nada de `flashcard_responses`/`flashcard_schedule`).
...
5. **Lista de coleções**: cada item com sigla/inicial da coleção (avatar tipo iniciais, ex: "DC" para Direito Constitucional), nome, contador de cards, seta de navegação (`>`). **Barra de progresso e percentual de acerto**: só pra coleções individuais/folha com pelo menos 1 card próprio — coleção-mãe (agrupa subcoleções) mostra texto "N cards · M subcoleções" no lugar da barra (mesmo tratamento já usado na tela de Coleções, reaproveitar, não duplicar lógica); coleção individual sem nenhum card ainda mostra "—" no lugar da barra vazia (evita barra 0% sem número, que parece bug em vez de estado vazio). **Três telas mostram lista de coleções com esse padrão de barra/percentual: Home ("Suas coleções"), Coleções (lista completa), e Progresso ("Evolução por coleção")** — as três precisam do mesmo tratamento pra coleção-mãe e coleção vazia, e idealmente compartilham o mesmo componente, não três implementações separadas.

**Decisão final — coleção-mãe fora da lista de evolução no Progresso**: "Evolução por coleção" no Progresso mostra especificamente desempenho/precisão por coleção — uma coleção-mãe não tem desempenho próprio (é só contêiner), então incluí-la ali não carrega informação real, só ocupa espaço. Decisão: **coleções-mãe não aparecem em "Evolução por coleção"** (Progresso), mas continuam aparecendo normalmente em "Suas coleções" (Home) e na tela de Coleções — nessas duas o propósito é navegação, não medição de desempenho, então a coleção-mãe com "N cards · M subcoleções" ainda é informação útil ali.
...
**Nota de implementação — tela de debug temporária (removida)**: `/upload/debug` foi criada durante o desenvolvimento do pipeline de extração (Estágio 1) para testar upload + polling de status + preview do conteúdo extraído, sem depender da UI final. Foi removida ao final do Estágio 3, depois de a UI final (GenerateWithAI → ReviewGeneratedCards) ter sido validada de ponta a ponta pelos dois modos (arquivo e tema).
...
- **Cursor-based**, não `OFFSET` — reaproveitar o desempate estável já existente (`criado_em, id`) como cursor: buscar os próximos N cards a partir do último `id` visto, não por número de página. Mais eficiente e mais correto que offset (que degrada com listas grandes e pode duplicar/pular itens se a lista mudar entre buscas).
- **Botão "Carregar mais"**, não infinite scroll automático — mais simples de implementar corretamente (sem `IntersectionObserver`, sem risco de disparar buscas demais ao rolar rápido), e adequado ao caso de uso (lista de estudo, não feed social).
- Tamanho de página inicial: definir um valor razoável (ex: 30-50 cards por carregamento) — não é uma decisão crítica, ajustar conforme sentir necessidade.
