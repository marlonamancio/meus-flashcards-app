...
6. **Modo de estudo**
   ...
   - **Retomar de onde parou (ajuste rápido, feedback real de uso)**: ao reabrir o modo de estudo de uma coleção onde a usuária fechou no meio antes (via botão X, sem terminar todos os cards), a sessão deve continuar do card seguinte ao último respondido, não recomeçar do primeiro card. Precisa de algum estado persistido indicando "quais cards desta coleção já foram vistos na passagem atual" — ao completar todos os cards da coleção (chegando ao fim), a próxima sessão reinicia do começo (nova "passagem" completa). Se novos cards forem adicionados à coleção no meio do caminho (via CSV/IA), definir se entram na passagem atual ou só na próxima — comportamento razoável: entram no fim da fila atual, não interrompem a ordem já em andamento.
   - **Dialog de escolha ao clicar "Estudar esta coleção"**: se existir uma passagem em andamento para aquela coleção (progresso salvo, sessão anterior fechada no meio), mostrar um modal perguntando: continuar de onde parou, ou começar do zero. "Começar do zero" descarta o progresso da passagem em andamento (reseta o estado de retomada para aquela coleção) e inicia uma passagem nova a partir do primeiro card. Se **não** houver passagem em andamento (coleção nunca estudada, ou última passagem já foi completada até o fim), pular o modal e ir direto para o estudo — o dialog só aparece quando há de fato uma escolha real a fazer.
   - **Navegação entre cards (Voltar / Pular)** — botões ao lado de "sabia"/"não sabia" (não gesto/swipe, decisão consciente para evitar acionamento acidental):
     - **Voltar**: mostra um card já respondido anteriormente na sessão atual, em modo somente-leitura (pergunta + resposta + o que foi marcado) — não permite responder de novo nem alterar o registro já salvo em `flashcard_responses`. É só revisão visual.
     - **Pular**: avança sem responder o card atual. O card pulado fica "pendente" e é **reenfileirado para reaparecer antes do fim da passagem atual** (não desaparece, não conta como visto para efeito de "retomar de onde parou" nem para streak/meta diária, já que só respostas de verdade contam para esses cálculos). Só marcar a passagem como completa quando todos os cards, incluindo os pulados, tiverem sido efetivamente respondidos.
...
- **Repetição espaçada (SM-2/FSRS) — algoritmo de agendamento de revisão.** Ideias já capturadas para quando essa etapa chegar (validadas por feedback real de uso — a usuária chegou independentemente na mesma necessidade):
  ...
  - **Modo de estudo "Recordação"**: segundo modo de estudo (além do padrão sequencial), exibindo cards aleatoriamente ou priorizando os com menor taxa de acerto primeiro. É essencialmente uma versão simplificada de repetição espaçada — não precisa esperar o algoritmo completo (SM-2/FSRS) para trazer valor real; pode ser um degrau intermediário antes da repetição espaçada "de verdade".
